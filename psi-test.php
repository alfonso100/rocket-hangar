	...
	<h4 id="step-3">2 - PSI Response</h4>
		...
		<div class="report">
			...
			//ROCKET PSI SCANNER	
			 ...
			 <p><strong>Based on Pagespeed recommendations, here I send you some specific performance optimization tips for your website:</strong></p>
			 <ul>


<li class="defer-offscreen-images hidden"><p><strong>Defer offscreen images</strong><br>Our <a href="https://docs.wp-rocket.me/category/32-lazyload">LazyLoad for Images</a> feature can usually help with this.<br><br>
...
<li class="avoid-an-excessive-dom-size hidden"><p><strong>Avoid an excessive DOM size</strong><br>This is something you would have to address directly through the design of your site.<br><br> 

A DOM element is something like a DIV, HTML, BODY, etc. element on a page -- in other words, these have to do with the HTML structure of your pages, so you'd have to actually change that structure outside WP Rocket in order to satisfy this suggestion.</p></li>



 <li class="avoid-serving-legacy-javascript-to-modern-browsers hidden"><p><strong>Avoid serving legacy JavaScript to modern browsers</strong><br>You should check the JavaScript files listed in this recommendation and send the following request to the respective plugin/theme/external service support team:<br><br>
...
This Opportunity is somewhat hard to accomplish but since its impact may not be noticeable for the visitor experience you could safely discard it.</p></li>


<li class="eliminate-render-blocking-resources hidden"><p><strong>Eliminate render blocking resources</strong><br>Enable the <strong>Optimize CSS delivery</strong> feature in the File Optimizations tab, and wait for the success message in the WP Rocket settings page in order to be sure it was applied to your site.</p></li>


				 

<li class="minify-css hidden">
<p><strong>Minify CSS</strong><br>Minifying CSS files basically consists in removing the comments and spaces that were added to the CSS files for readability purposes, making the CSS files smaller.<br><br>
...
<li class="minify-javascript hidden"><p><strong>Minify JavaScript</strong><br>JavaScript minification consists on removing the unnecessary information from the scripts, things like spacing and developer comments are deleted when files are minified.
...
<li class="properly-size-images hidden"><p><strong>Properly size images</strong><br>This means you're using images that are bigger than the space in which they are displayed, forcing the browser to resize them.<br>
So, the way to solve the problem would be to use images that are exactly the same size as they are presented in the various sections of your page.</p></li>



<li class="preload-key-requests hidden"><p><strong>Preload key requests</strong><br> According to what Google explains <a href="https://developers.google.com/web/tools/lighthouse/audits/preload">here</a> they want you to add <em>rel=preload</em> attribute to the LINK tag.<br><br>

For font files, since WP Rocket v3.6, you can add them in the <strong>Preload Fonts</strong> section (from the Preload tab: https://share.getcloudapp.com/qGuK8J9w).<br><br>

You can find more info about how to use the feature in <a href="https://docs.wp-rocket.me/article/1317-preload-fonts">this link</a>.</p></li>
				 


<li class="remove-unused-javascript hidden"><p><strong>Remove unused JavaScript</strong><br>Our feature, <strong><em>Delay JavaScript Execution</em></strong> can help. By delaying the files, PageSpeed will no longer warn you about them. However, not every file can be safely delayed. You should only delay files which are not needed for anything “above the fold”. <a href="https://docs.wp-rocket.me/article/1349-delay-javascript-execution">Please see this doc</a>.<br>
...
<ul><li>https://wordpress.org/plugins/plugin-organizer/</li>
<li>https://wordpress.org/plugins/wp-asset-clean-up/</li>
<li>https://tomasz-dobrzynski.com/wordpress-gonzales</li></ul></p></li>
				 


<li class="remove-unused-css hidden"><p><strong>Remove unused CSS</strong><br>The description for "Remove unused CSS" says, <em>"Remove unused rules from stylesheets to reduce unnecessary bytes consumed by network activity"</em>.<br><br>
...
By activating the <strong>Optimize CSS Delivery</strong> option, WP Rocket defers the loading of CSS, but we cannot remove the unused parts.<br><br>
...
<li class="serve-images-in-next-gen-formats hidden"><p><strong>Serve images in next-gen formats</strong><br>You can consider converting images to WebP format with some plugins. Depending on the settings in the image optimization plugin you may or may not need to activate WebP cache in WP Rocket. Here are more details on that: <a href="https://docs.wp-rocket.me/article/1282-webp">WebP Compatibility</a>

We have built-in compatibility with <a href="https://wordpress.org/plugins/imagify/">Imagify plugin</a> and some other. Sometimes the plugin can deliver the WebP images itself. In this case, you don't need to activate WebP cache option in WP Rocket.</p></li>
				 
				

<li class="serve-static-assets-with-an-efficient-cache-policy hidden"><p><strong>Serve static assets with an efficient cache policy</strong><br>Try enabling:<br>
...
They can be found in the Add-on tab in WP Rocket.</p></li>		



<li class="reduce-initial-server-response-time hidden"><p><strong>Reduce initial Server Response Time</strong><br>WP Rocket's page caching will definitely improve the server response time.<br><br>
...
https://docs.wp-rocket.me/article/103-my-website-is-slow</p></li>
 	...
 	<small>From this list, add some of the <strong>Opportunities</strong> from the Pagespeed report gave you for the <em>cached</em> version of the website.</small>
...
<br><br>
			...
			<?php 
				...
				$psi_opportunities[] = 'Eliminate render blocking resources';
				$psi_opportunities[] = 'Avoid serving legacy JavaScript to modern browsers';
				$psi_opportunities[] = 'Serve static assets with an efficient cache policy';
				$psi_opportunities[] = 'Reduce initial Server Response Time';
				$psi_opportunities[] = 'Avoid an excessive DOM size';
				$psi_opportunities[] = 'Defer offscreen images';
				...
				asort($psi_opportunities);
				
				foreach($psi_opportunities as $psi_opportunity) :
					
					$cleaned_psi = str_replace(' ', '-', strtolower($psi_opportunity));
					echo '<div class="form-check">
						  <input class="form-check-input psicheck" type="checkbox" value="'.$cleaned_psi.'" id="'.$cleaned_psi.'" name="psi[]">
						  <label class="form-check-label" for="'.$cleaned_psi.'"> '.$psi_opportunity.'
						  </label>
						</div>';
...
<div class="row px-4 py-0">
	<div class="col-6">
 	...
 	<small>This is the current WP Rocket's benefit on your website. You can see the report for the cached version of your website on the left, and the non-cached version on the right. If you scroll down at the cached version, you can find the top <strong>Opportunities and Recommendations</strong>, these are the ones  you can use on <a href="#step-3">Step #3</a> to get add some customized performance recommendations.</small>
